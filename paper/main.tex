...
\newcommand{\FqSix}{\ensuremath{\mathbb{F}_{q^6}} }
...
Elliptic curve pairings represent fundamental building blocks in modern cryptographic protocols, serving as the mathematical foundation for numerous cryptographic systems including Groth16\cite{Gro16}, PLONK\cite{plnk19}, BLS signature schemes\cite{bls05}, and KZG polynomial commitment schemes\cite{kzg10}. At their core, elliptic curve pairings provide a bilinear map\cite{Ara13} $e: \mathbb{G}_1 \times \mathbb{G}_2 \rightarrow \mathbb{G}_T$ that enables a form of an encrypted multiplication over elliptic curve groups. This bilinearity property — where $e(aP, Q) = e(P, aQ) = e(P, Q)^a$ for scalar $a$ and points $P \in \mathbb{G}_1$, $Q \in \mathbb{G}_2$ — has enabled breakthrough advances across cryptographic applications, from identity-based encryption to the zero-knowledge systems securing modern blockchain infrastructure.
...
\hline
\multicolumn{4}{|c|}{} \\
\multicolumn{4}{|c|}{\textbf{Native BLS12315 \cite{sf2c22} pairing}} \\
\hline
...
This algorithm combines residue witness \cite{OPP24} and Feltroidprime's \FqTwelve arithmetic.

\begin{algorithm}
\caption{Optimal Ate pairing with Residue witness}
\label{alg:pairing}
\begin{algorithmic}[1]
\Require $P \in \G_1, Q \in \G_2, c \in \FqTwelve, w \in \{0,1,2\}$
\Statex $\text{Internal } W, W^2 \in \FqTwelve$ \Comment{$W$ is 27th root of unity}
\Ensure $a_{opt}(Q, P)$
\State Write $s = 6t + 2 = \sum_{i=0}^{L-1} s_i 2^i$ where $s_i \in \{-1, 0, 1\}$ and $s_L = 1$
...
\begin{enumerate}
    \item Operate in a 3-pairing Miller loop setting.
    \item Prepare the polynomial relationships to assert for a 0-bit operation.
    \item We skip the evaluations and commitments of quotient polynomials \footnote{The evaluation and commitment of quotient polynomials $Q$ is skipped because they are aggregated into a single polynomial commitment at the end of the computation amortised across all operations.}
    \item Look at the Feltroidprime's implementation \cite{FXFM23}
    \item Compare it with our approach
\end{enumerate}

\subsubsection{Previous scheme}

\textbf{Adapting faster extension field multiplication \cite{FXFM23}} idea to line 4 of Miller loop algorithm \ref{alg:pairing} for a 3--pair Miller loop, we have following polynomial relationships to assert.
...
$F$, $T_1$, $T_2$, $T_3$, $R$ cost 12C each for evaluation, 4C for $\Fq$ multiplication, and $L_1$, $L_2$, $L_3$ require 4C each for evaluaiton. All the remainder hints need to be provided for commitment, so this is 12 calldata commitment for $R$ and intermediate remainder polynomials $T_1$, $T_2$, $T_3$. \textbf{Costs sum up to 76C} ($\text{ } 5 * 12C \text{ } + \text{ } 3 * 4C \text{ } + \text{ } 4C \text{ }$) \textbf{and 48 commitments} in $\Fq$ ($4 * 12$). Resulting remainder polynomial $R \in \FqTwelve$ is the result of the Miller loop step.

\subsubsection{Our scheme}

\textbf{In our approach,} instead of working with individual arithmetic operations, we treat the entire bit operation as a single polynomial. In our scheme line 4 of  3--pair Miller loop (Algorithm \ref{alg:pairing}) looks like this.
