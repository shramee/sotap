...
Squaring operations \textbf{double the polynomial degree}. In traditional $\Fe{12}$ arithmetic, squaring operations are typically cheaper than general multiplications. However, in our polynomial verification framework, squaring the accumulator $F$ doubles the polynomial degree. Placed incorrectly it becomes the most degree-expensive operation.
...
This observation naturally leads to segmenting operations at each Miller loop step boundary, where the accumulator $F$ is squared. The remaining operations—initial setup and final exponentiation verification—are then segmented to match the degree profile of Miller loop step polynomials, ensuring balanced verification costs across all phases.

\subsection{Unified Polynomial Framework}

The key insight is that any sequence of field operations in $\Fe{12}$ \footnote{Elements need not have all 12 coefficients as an \Fe{12} but should be in the same form such that the coefficients align with the \Fe{12} representation.} can be expressed as a single polynomial equation of the form:

$$A_1(x) \cdot \ldots \cdot A_n(x) = Q(x) \cdot P_{12}(x) + R(x) \text{ for } A_1, ..., A_n \in \Fe{12}$$
