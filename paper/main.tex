...
This is a little obvious optimisation, not included in the paper, but when operations are queued consecutively, the evaluated $R_i(z)$ can be used as $A_{i+1}$ for the subsequent operation, and we get a free evaluation. So for Squaring and 3 sparse multiplications, we can reuse the evaluation of $R_{i-1}$ as $A_i$ for sparse multiplications saving 12C on each sparse multiplication.
...
\hline
 & Mul & Square & Div & sparse\_Mul & Sq + 3 sparse \\
\hline
$\mathbb{F}_{q^{12}}$ & 39C & 27C & 51C & 31C & 84C \\
\hline
\end{tabular}
\end{table}
