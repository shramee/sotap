...
\subsubsection{Queued operations}

When operations are queued consecutively, and evaluated $R_i(z)$ can be used as $A_{i+1}$ for the subsequent operation, we get a free evaluation. So for Squaring and 3 sparse multiplications, we can reuse the evaluation of $R_{i-1}$ as $A_i$ for each of sparse multiplication.
...
\hline
 & Mul & Square & Div & sparse\_Mul & Sq + 3 sparse \\
\hline
$\mathbb{F}_{q^{12}}$ & 39C & 27C & 51C & 31C & 27C \\
\hline
\end{tabular}
\end{table}
