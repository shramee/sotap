...
The code is available at \url{https://anonymous.4open.science/r/tiny-gnark-DDCD/}.
...
rem := acc.Eval()
\end{verbatim}
\texttt{PolyRingAccumulator} maintains a queue of polynomial factors representing
the deferred product $\prod_i \mathtt{state}_i$. Calling \texttt{Mul} appends a
factor; calling \texttt{Sqr} duplicates every current factor so that the next
\texttt{Eval} computes $(\prod_i \mathtt{state}_i)^2$. When \texttt{targetDeg} is
set, \texttt{Eval} is triggered automatically whenever the accumulated degree
would exceed the bound, keeping intermediate products within a manageable degree
before the final \texttt{MulPolyRings} call. Without a target degree all
operations are deferred until \texttt{Eval} is called explicitly.
