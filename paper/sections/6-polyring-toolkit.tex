...
Batching all quotients into a single $Q_{\mathrm{acc}}$ is the key saving: only
one polynomial is committed instead of $s$, giving the 75\% calldata reduction
noted in Section~\ref{sec:op:comp:our}.
...
\end{verbatim}
Implements Equation~\eqref{eq:iop-batched} via two \texttt{Committer.Commit}
calls (producing challenges $z$ and $x$) and one \texttt{AssertIsEqual} per
group. No in-circuit work beyond remainder range-checks is performed until
this point.
...
and calls \texttt{Committer.Commit} to obtain $z$ in the circuit.
...
%
The $Q_{\mathrm{acc}}$ limbs (together with $z$) are collected and passed to
\texttt{Committer.Commit} to obtain $x$.
