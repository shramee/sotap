...
calls~\cite{gnarkcommit}: the first commits the queued remainders and pairing
inputs to derive the RLC challenge $z$; the second commits $Q_\text{acc}$
(computed by \texttt{quotientsRLCHint}) to derive the evaluation point $x$.

\begin{table}[ht]
\centering
\caption{Paper-to-implementation mapping (paths relative to the anonymised repository)}
\label{tab:alg_mapping}
\begin{tabular}{|l|l|}
\hline
\textbf{Paper artefact} & \textbf{Implementation} \\
\hline
Alg.~\ref{alg:polymuldiv} \textsc{PolyMulDiv} & \texttt{polyRingMulHint} (out-of-circuit hint) \\
\hline
Alg.~\ref{alg:2r:prover} prover loop & Miller loop, \texttt{sw\_bn254/pairing.go}; \\
 (arrays $\mathcal{Q}, \mathcal{R}$) & queued in \texttt{f.deferredPolyChecks} \\
\hline
Prover RLC $Q_\text{acc} = \sum_i z^i Q_i$ & \texttt{quotientsRLCHint} \\
\hline
Alg.~\ref{alg:2r:verifier} + Eq.~\eqref{eq:iop-batched} & \texttt{performDeferredRingChecks}, \\
 & \texttt{emulated/field\_polyring.go} \\
\hline
Challenges $z$, $x$ (Fiat-Shamir, \S\ref{sec:iop:fs}) & two sequential \texttt{api.Commit} calls~\cite{gnarkcommit} \\
\hline
\end{tabular}
\end{table}
...
products of two $b$-bit values, so its magnitude is bounded by $\beta = 2^{2b + \lceil\log_2(d+1)\rceil}$.
