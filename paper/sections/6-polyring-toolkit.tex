...
\begin{lemma}[Overflow safety for BN254 pairing polynomials]\label{lem:overflow}
With 4-limb, 64-bit emulated arithmetic and $d \leq 256$, each result-limb accumulates
at most $4(d{+}1) \leq 1028$ products of two $2^{64}$-bounded values, giving a maximum
limb magnitude of $1028 \times 2^{128} < 2^{138}$.
For the $Q_{\mathrm{acc}}(x)\cdot P_{12}(x)$ product, the sparse $P_{12}$ (3 nonzero
coefficients, largest $82 < 2^7$) raises this to $< 2^{146}$.
Since $2^{146} \ll p \approx 2^{254}$, no intermediate limb reaches the native
characteristic, so the deferred single-element reduction is safe for all polynomial
evaluations in BN254 pairing verification.
\end{lemma}