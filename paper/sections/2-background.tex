...
While we specifically implement our library in Cairo, we map our MULMOD operation to a constraint (denoted by \textbf{C}) to maintain compatibility with the Rank-1 Constraint System (R1CS) model to benchmark against established literature. In R1CS~\cite{Gro16}, prover complexity is dominated by multiplication gates while additions are free. In the Cairo VM, this corresponds to the Algebraic Intermediate Representation (AIR) where operations are verified through polynomial constraints. To synchronize our data with the benchmarks provided by El Housni~\cite{PR1CS22}, we define our cost metric based on field multiplications.
