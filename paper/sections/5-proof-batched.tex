...
\textbf{Concrete commitment costs:} In gnark, each Fiat-Shamir challenge is produced by a \texttt{Committer.Commit} call, which contributes to the circuit's commitment count---the dominant overhead in recursive proof settings. Our batching strategy reduces the number of \texttt{Commit} calls from $O(k \cdot n)$ to $O(k)$, where $k$ is the Miller loop length and $n$ is the number of pairings. For BN254 2-pairing verification, this yields the 44\% reduction in total commitments observed in Table~\ref{tab:main_performance}.
