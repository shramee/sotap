...
The verifier recieves computed remainders $\mathcal{R}$ for each polynomial equation described in Section \ref{sec:op-polys} from the prover. Along the array of remainders, it retains the challenge $c$ provided to the prover after the first round. Along with point pairs $P \in \G_1$ and $Q \in \G_2$, the residue witness $c$ as computed in \cite{OPP24}, internal parameter $W$ (the 27th root of unity) and $Q_\text{acc}$, the verifier runs the pairing computation while using the remainders and performs polynomial evaluations (Algo \ref{alg:evalpolyprod}) at each step. This process is detailed in Algorithm \ref{alg:2r:verifier}.
